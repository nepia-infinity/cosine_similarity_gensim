...
<img width="900" height="1230" alt="image" src="https://github.com/user-attachments/assets/1b845d91-aa84-41d7-a574-39a7d06af788" />
...
- `東京都` (名詞), `に` (助詞), `住ん` (動詞), `で` (助詞), `い` (動詞), `ます` (助動詞)
...
マルチラベル分類（Multi-Label Classification）とは、機械学習の手法の一つです。

1つのデータ（例: 1件の問い合わせ）に対して、関連する複数の正解ラベル（例: タグ）を同時に予測・割り当てる ことを目指します。

### 🔑 「多クラス分類」との違い
- 初学者が混同しやすい「多クラス分類」との違いは、**「正解が1つか、複数か」**です。

### 多クラス分類 (Multi-Class)
- たくさんの選択肢から「1つだけ」を選びます。
- 例: 1枚の画像を「犬」「猫」「鳥」のどれか1つに分類する。

### マルチラベル分類 (Multi-Label)
- たくさんの選択肢から「当てはまるもの全部」を選びます。
- 例: 1本の映画に「アクション」「コメディ」「SF」という3つのタグを同時に付ける。

## 📝 Jaccard（ジャッカード）係数とは？
2つの「集合（＝キーワードや単語の集まり）」が どれだけ似ているか を数値で表す指標です。
...
## キーワード一致率の算出
| No. | Text 1              | Text 2          | 共通キーワード                   | 全キーワード数 | Jaccard係数  |
| --- | ------------------- | --------------- | ------------------------- | ------- | ---------- |
| 1   | パソコンが起動しない          | PCが立ち上がらない      | {'起動不可', 'パソコン'}          | 2       | **1.0000** |
| 2   | パソコンが起動しない          | ノートパソコンの画面が映らない | {'パソコン'}                  | 4       | **0.2500** |
| 3   | Windowsのアップデートに失敗した | windowsの更新ができない | {'Windows', '更新', 'パソコン'} | 3       | **1.0000** |

> 💡 **Jaccard係数** は 0〜1 の範囲で表され、
> 値が1に近いほど「キーワードの集合が似ている（＝意味が近い）」ことを示します。
...
## AIエージェントの活用
- READMEの更新に、[Gemini CLI](https://drive.google.com/file/d/1LcwIhYvGEgjJ_g7m777P3gfDA5iJxebr/view?usp=sharing)を利用しました。
